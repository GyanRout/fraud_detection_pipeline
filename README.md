...
models/: Contains the compiled fraud_model_traced.pt C++ artifact.

src/ml/inference.py: The singleton inference engine that warms up the JIT compiler and executes zero-copy tensor evaluations.

src/core/database.py: SQLAlchemy 2.0 ORM and asynchronous connection pooling.

src/api/websocket.py: Thread-safe broadcaster for real-time threat ledgers.

frontend/: The stateful React dashboard visualizing network anomalies.
...
In a separate terminal, generate synthetic financial traffic to watch the pipeline in action:
